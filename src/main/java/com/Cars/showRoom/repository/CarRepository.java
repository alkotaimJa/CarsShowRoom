...
public interface CarRepository extends JpaRepository<Car, Long> {
    @Query("SELECT new com.Cars.showRoom.DTO.CarShowroomDetailsDTO(c.vin, c.maker, c.model, c.model_year, c.price, cs.name, cs.contact_number) " +
    ...
    "AND (:modelYear IS NULL OR c.model_year = :modelYear) " + 
    "AND (:showroomName IS NULL OR cs.name LIKE %:showroomName%) " +
    "AND (:contactNumber IS NULL OR cs.contact_number LIKE %:contactNumber%)")
    Page<CarShowroomDetailsDTO> findAllWithShowroomDetails(Pageable pageable, 
